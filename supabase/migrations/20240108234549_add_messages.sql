...
BEGIN
  SELECT COALESCE(MAX(sequence_number), 0) + 1 INTO _sequence_number FROM messages WHERE chat_id = p_chat_id;
...
--   INSERT INTO messages (chat_id, user_id, model, process_name, message_type, role, image_paths, sequence_number, name, content, tool_calls, tool_call_id, active)
...
--   process_name := _message.process_name;
--   message_type := _message.message_type;
--   role := _message.role;
--   image_paths := _message.image_paths;
--   sequence_number := _message.sequence_number;
--   name := _message.name;
--   content := _message.content;
--   tool_calls := _message.tool_calls;
--   tool_call_id := _message.tool_call_id;
--   active := _message.active;
